    ...
    <section class="clean-block clean-form dark h-100">
        <div class="container" style="margin-top: -59px;">
            <div class="block-heading" style="padding-top: 0px;">
                <h2 class="text-primary" style="margin-top: 100px;">Registrar foto de producto</h2>
                <p>Realiza el registro  llenando los siguientes datos</p>
            </div>
            {!! Form::open([ 'method' => 'PATCH' , 'url'=>'/fotos_productos/'.$foto_producto->id ,'enctype'=>'multipart/form-data']) !!}
            <div class="form-group mb-3">
                <label class="form-label" for="producto">producto</label>
                <select class="form-select states order-alpha" id="id_producto" name="id_producto" required="" disabled>
                    @foreach($productos as $producto)
                        <option value="{{ $producto->id }}" @if($producto->id == $foto_producto->id_prdocuto) selected @endif>{{ $producto->nombre }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group mb-3">
                ...
                {!! Form::file('foto', ['accept' => '.jpg,.jpeg,.bmp,.png,.doc,.docx', 'required' => 'required']) !!}
            </div>
            <hr style="margin-top: 30px;margin-bottom: 10px;">
            <div class="form-group mb-3">&nbsp;{!! Form::submit('Editar tienda', ['class' => 'btn btn-primary d-block w-100']) !!}</div>
            <a href="{{ url('/fotos_productos') }}" class="btn btn-primary d-block w-100" style="background: rgb(237,38,38);">Cancelar</a>
            {!! Form::close() !!}
        </div>
    </section>
