...
<body>
    ...
    <section class="clean-block clean-form dark h-100">
        <div class="container" style="margin-top: -59px;">
            ...
            {!! Form::open([ 'method' => 'PATCH' , 'url'=>'/mermas/'.$merma->id]) !!}
                <div class="form-group mb-3">{!! Form::label ('fecha','Fecha', ['class' => 'form-label']) !!}{!! Form::date('fecha', $merma->fecha, ['class' => 'form-control', 'placeholder' => 'Ingresa Fecha', 'required' => 'required', 'title' => 'Debes ingresar el Fecha']) !!}</div>
                <div class="form-group mb-3">{!! Form::label ('total','Total', ['class' => 'form-label']) !!}{!! Form::number('total', $merma->total, ['class' => 'form-control', 'placeholder' => 'Ingresa Total', 'required' => 'required', 'title' => 'Debes ingresar el Total']) !!}</div>
                <div class="form-group mb-3">
                    <label class="form-label" for="usuario">Usuario</label>
                    <select class="form-select states order-alpha" id="usuario" name="id_usuario" required="">
                        @foreach($usuarios as $usuario)
                            <option value="{{ $usuario->id }}" @if($usuario->id == $merma->id_usuario) selected @endif >{{ $usuario->nombre . " " .  $usuario->ap_pat . " " . $usuario->ap_mat}}</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group mb-3">
                    ...
                    <select class="form-select states order-alpha" id="tienda" name="id_tienda" required="">
                        @foreach($tiendas as $tienda)
                            <option value="{{ $tienda->id }}" @if($tienda->id == $merma->id_tienda) selected @endif >{{ $tienda->nombre }}</option>
                        @endforeach
                    </select>
                </div>
                <hr style="margin-top: 30px;margin-bottom: 10px;">
                <div class="form-group mb-3">&nbsp;{!! Form::submit('Editar merma', ['class' => 'btn btn-primary d-block w-100']) !!}</div>
                <a href="{{ url('/mermas') }}" class="btn btn-primary d-block w-100" style="background: rgb(237,38,38);">Cancelar</a>
            {!! Form::close() !!}
        </div>
    </section>
