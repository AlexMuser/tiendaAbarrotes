    ...
    <section class="clean-block clean-form dark h-100">
        <div class="container" style="margin-top: -59px;">
            <div class="block-heading" style="padding-top: 0px;">
                <h2 class="text-primary" style="margin-top: 100px;">Registrar detalle de ventas</h2>
                <p>Realiza el registro llenando los siguientes datos</p>
            </div>
            {!! Form::open([ 'method' => 'PATCH' , 'url'=>'/detalle_ventas/'.$detalle_venta->id]) !!}
                <div class="form-group mb-3">{!! Form::label ('cantidad','Cantidad', ['class' => 'form-label']) !!}{!! Form::number('cantidad', $detalle_venta->cantidad, ['class' => 'form-control', 'placeholder' => 'Ingresa cantidad', 'required' => 'required', 'title' => 'Debes ingresar el cantidad']) !!}</div>
                <div class="form-group mb-3">{!! Form::label ('precio_compra','Precio de compra', ['class' => 'form-label']) !!}{!! Form::number('precio_compra', $detalle_venta->precio_compra, ['class' => 'form-control', 'placeholder' => 'Ingresa el precio_compra', 'required' => 'required', 'title' => 'Debes ingresar el precio de compra']) !!}</div>
                <div class="form-group mb-3">{!! Form::label ('precio_venta','Precio de venta', ['class' => 'form-label']) !!}{!! Form::number('precio_venta', $detalle_venta->precio_venta, ['class' => 'form-control', 'placeholder' => 'Ingresa el precio_compra', 'required' => 'required', 'title' => 'Debes ingresar el precio de venta']) !!}</div>
                <div class="form-group mb-3">
                    <label class="form-label" for="venta">Venta</label>
                    <select class="form-select states order-alpha" id="venta" name="id_venta" required="">
                        @foreach($ventas as $venta)
                            <option value="{{ $venta->id }}" @if($venta->id == $detalle_venta->id_venta) selected @endif >{{ ($venta->id)}}</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group mb-3">
                    ...
                    <select class="form-select states order-alpha" id="producto" name="id_producto" required="">
                        @foreach($productos as $producto)
                            <option value="{{ $producto->id }}" @if($producto->id == $detalle_venta->id_producto) selected @endif >{{ $producto->nombre }}</option>
                        @endforeach
                    </select>
                </div>
                <hr style="margin-top: 30px;margin-bottom: 10px;">
                <div class="form-group mb-3">&nbsp;{!! Form::submit('Guardar detalle ventas', ['class' => 'btn btn-primary d-block w-100']) !!}</div>
                <a href="{{ url('/detalle_ventas') }}" class="btn btn-primary d-block w-100" style="background: rgb(237,38,38);">Cancelar</a>
            {!! Form::close() !!}
        </div>
    </section>
