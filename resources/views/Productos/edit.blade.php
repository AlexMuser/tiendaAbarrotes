    ...
    <section class="clean-block clean-form dark h-100">
        <div class="container" style="margin-top: -59px;">
            ...
            {!! Form::open([ 'method' => 'PATCH' , 'url'=>'/productos/'.$producto->id]) !!}
                <div class="form-group mb-3">{!! Form::label ('nombre','Nombre', ['class' => 'form-label']) !!}{!! Form::text('nombre', $producto->nombre, ['class' => 'form-control', 'placeholder' => 'Ingresa el nombre', 'required' => 'required', 'title' => 'Debes ingresar el nombre']) !!}</div>
                <div class="form-group mb-3">{!! Form::label ('descripcion','Descripción', ['class' => 'form-label']) !!}{!! Form::text('descripcion', $producto->descripcion, ['class' => 'form-control', 'placeholder' => 'Ingresa la descripción', 'required' => 'required', 'title' => 'Debes ingresar la descripción']) !!}</div>
                <div class="form-group mb-3">{!! Form::label ('codigo','Codigo', ['class' => 'form-label']) !!}{!! Form::number('codigo', $producto->codigo, ['class' => 'form-control', 'placeholder' => 'Ingresa el codigo', 'required' => 'required', 'title' => 'Debes ingresar el codigo']) !!}</div>
                <div class="form-group mb-3">{!! Form::label ('existencia','Numero de existencias', ['class' => 'form-label']) !!}{!! Form::number('existencia', $producto->existencia, ['class' => 'form-control', 'placeholder' => 'Ingresa el numero de existencias', 'required' => 'required', 'title' => 'Debes ingresar el numero de existencias']) !!}</div>
                <div class="form-group mb-3">{!! Form::label ('precio_compra','Precio de compra $', ['class' => 'form-label']) !!}{!! Form::number('precio_compra', $producto->precio_compra, ['class' => 'form-control', 'placeholder' => 'Ingresa el precio de compra', 'required' => 'required', 'title' => 'Debes ingresar el precio de compra']) !!}</div>
                <div class="form-group mb-3">{!! Form::label ('precio_venta','Precio de venta $', ['class' => 'form-label']) !!}{!! Form::number('precio_venta', $producto->precio_venta, ['class' => 'form-control', 'placeholder' => 'Ingresa el precio de venta', 'required' => 'required', 'title' => 'Debes ingresar el precio de venta']) !!}</div>
                <div class="form-group mb-3">{!! Form::label ('stock','Cantidad en stock', ['class' => 'form-label']) !!}{!! Form::number('stock', $producto->stock, ['class' => 'form-control', 'placeholder' => 'Ingresa el stock', 'required' => 'required', 'title' => 'Debes ingresar el stock']) !!}</div>
                <div class="form-group mb-3">
                    ...
                    <select class="form-select states order-alpha" id="tipos_ventas" name="id_tipo_venta" required="">
                        @foreach($tipos_ventas as $tipo_venta)
                            <option value="{{ $tipo_venta->id }}" @if($tipo_venta->id == $producto->id_tipo_venta) selected @endif>{{ $tipo_venta->nombre }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group mb-3">
                ...
                <div class="form-group mb-3">
                    ...
                    <select class="form-select states order-alpha" id="id_proveedor" name="id_proveedor" required="">
                        @foreach($proveedores as $proveedor)
                            <option value="{{ $proveedor->id }}" @if($proveedor->id == $producto->id_proveedor) selected @endif>{{ $proveedor->nombre }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group mb-3">
                    ...
                    <select class="form-select states order-alpha" id="id_tienda" name="id_tienda" required="">
                        @foreach($tiendas as $tienda)
                            <option value="{{ $tienda->id }}"  @if($tienda->id == $producto->id_tienda) selected @endif>{{ $tienda->nombre }}</option>
                        @endforeach
                    </select>
                </div>
