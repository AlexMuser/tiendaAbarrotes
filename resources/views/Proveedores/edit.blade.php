    ...
    <section class="clean-block clean-form dark h-100">
        <div class="container" style="margin-top: -59px;">
            <div class="block-heading" style="padding-top: 0px;">
                <h2 class="text-primary" style="margin-top: 100px;">Registrar proveedores</h2>
                <p>Realiza el registro  llenando los siguientes datos</p>
            </div>
            {!! Form::open([ 'method' => 'PATCH' , 'url'=>'/proveedores/'.$proveedor->id]) !!}
                <div class="form-group mb-3">{!! Form::label ('nombre','Nombre', ['class' => 'form-label']) !!}{!! Form::text('nombre', $proveedor->nombre, ['class' => 'form-control', 'placeholder' => 'Ingresa el nombre', 'required' => 'required', 'title' => 'Debes ingresar el nombre']) !!}</div>
                <div class="form-group mb-3">{!! Form::label ('telefono','Teléfono', ['class' => 'form-label']) !!}{!! Form::tel('telefono', $proveedor->telefono, ['class' => 'form-control', 'placeholder' => 'Ingresa el teléfono', 'required' => 'required', 'title' => 'Debes de ingresar un numero de teléfono']) !!}</div>
                <div class="form-group mb-3">
                    {!! Form::label('dia_visita', 'Día de visita', ['class' => 'form-label']) !!}
                    {!! Form::select('dia_visita', ['Lunes' => 'Lunes', 'Martes' => 'Martes', 'Miércoles' => 'Miércoles', 'Jueves' => 'Jueves', 'Viernes' => 'Viernes'], $proveedor->dia_visita, ['class' => 'form-control', 'placeholder' => 'Selecciona un día de la semana', 'required' => 'required']) !!}
                </div>
                ...
                <div class="form-group mb-3">
                    ...
                    <select class="form-select states order-alpha" id="tienda" name="id_tienda" required="">
                        @foreach($tiendas as $tienda)
                            <option value="{{ $tienda->id }}" @if($tienda->id == $proveedor->id_tienda) selected @endif>{{ $tienda->nombre }}</option>
                        @endforeach
                    </select>
                </div>
                <hr style="margin-top: 30px;margin-bottom: 10px;">
                <div class="form-group mb-3">&nbsp;{!! Form::submit('Editar tienda', ['class' => 'btn btn-primary d-block w-100']) !!}</div>
                <a href="{{ url('/proveedores') }}" class="btn btn-primary d-block w-100" style="background: rgb(237,38,38);">Cancelar</a>
                {!! Form::close() !!}
        </div>
    </section>
