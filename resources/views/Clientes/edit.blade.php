...
<body>
    <section class="clean-block clean-form dark h-100">
        <div class="container" style="margin-top: -59px;">
            ...
            {!! Form::open([ 'method' => 'PATCH' , 'url'=>'/clientes/'.$cliente->id]) !!}
                <div class="form-group mb-3">{!! Form::label ('ap_pat','Apellido Paterno', ['class' => 'form-label']) !!}{!! Form::text('ap_pat', $cliente->ap_pat, ['class' => 'form-control', 'placeholder' => 'Ingresa apellido paterno', 'required' => 'required', 'title' => 'Debes ingresar el apellido paterno']) !!}</div>
                <div class="form-group mb-3">{!! Form::label ('ap_mat','Apellido Materno', ['class' => 'form-label']) !!}{!! Form::text('ap_mat', $cliente->ap_mat, ['class' => 'form-control', 'placeholder' => 'Ingresa apellido materno', 'required' => 'required', 'title' => 'Debes ingresar el apellido materno']) !!}</div>
                <div class="form-group mb-3">{!! Form::label ('nombre','Nombre', ['class' => 'form-label']) !!}{!! Form::text('nombre', $cliente->nombre, ['class' => 'form-control', 'placeholder' => 'Ingresa el nombre', 'required' => 'required', 'title' => 'Debes ingresar el nombre']) !!}</div>
                <div class="form-group mb-3">{!! Form::label ('telefono','Teléfono', ['class' => 'form-label']) !!}{!! Form::tel('telefono', $cliente->telefono, ['class' => 'form-control', 'placeholder' => 'Ingresa el teléfono', 'required' => 'required', 'title' => 'Debes de ingresar un numero de teléfono']) !!}</div>
                <div class="form-group mb-3">{!! Form::label ('direccion','Dirección', ['class' => 'form-label']) !!}{!! Form::text('direccion', $cliente->direccion, ['class' => 'form-control', 'placeholder' => 'Ingresa la dirección', 'required' => 'required', 'title' => 'Debes de ingresar una dirección']) !!}</div>
                <div class="form-group mb-3">
                    ...
                    <select class="form-select states order-alpha" id="tienda" name="id_tienda" required="">
                        ...
                        <option value="{{ $tienda->id }}" @if($tienda->id == $cliente->id_tienda) selected @endif>{{ $tienda->nombre }}</option>
                        @endforeach
                    </select>
                </div>
                <hr style="margin-top: 30px;margin-bottom: 10px;">
                <div class="form-group mb-3">&nbsp;{!! Form::submit('Editar cliente', ['class' => 'btn btn-primary d-block w-100']) !!}</div>
